...
if not os.path.exists(CREDENTIALS_KEY_FILE):
    # 0o600 — the AES key that decrypts all *_credentials.bin files. Without
    # this, on a shared host any local user can read the key and decrypt the
    # Garmin / Zwift / Intervals / COROS / Strava credential blobs.
    with open(CREDENTIALS_KEY_FILE, 'wb') as f:
        os.chmod(CREDENTIALS_KEY_FILE, 0o600)
...
# Whitelist of filenames the /delete/<name> route is allowed to remove. Kept
# as a module-level constant so the writer side (coros/garmin/zwift/intervals
# settings pages) and the delete side can never drift. Adding a new
# integration? Add the filename here AND in the corresponding upload function.
_DELETABLE_PROFILE_FILES = ['profile.bin', 'achievements.bin']
_DELETABLE_CREDENTIALS = [
    'garmin_credentials.bin',
    'zwift_credentials.bin',
    'intervals_credentials.bin',
    'coros_credentials.bin',
]
_DELETABLE_STRAVA = ['strava_api.bin', 'strava_token.txt']
DELETABLE_FILENAMES = set(_DELETABLE_PROFILE_FILES + _DELETABLE_CREDENTIALS + _DELETABLE_STRAVA)


@app.route("/delete/<filename>", methods=["GET"])
@login_required
def delete(filename):
    if filename not in DELETABLE_FILENAMES:
        return '', 403
    delete_file = os.path.join(STORAGE_DIR, str(current_user.player_id), filename)
    if os.path.isfile(delete_file):
        os.remove("%s" % delete_file)
    if filename in _DELETABLE_STRAVA:
        return redirect(url_for('strava', username=current_user.username))
    if filename in _DELETABLE_CREDENTIALS:
...
def strava_upload(player_id, activity):
    ...
    try:
        ...
        logger.exception("Strava upload failed. No internet?")
...
def garmin_upload(player_id, activity):
    ...
    try:
        ...
        # `except Exception` (not bare `except:`) — a bare except would
        # also catch SystemExit / KeyboardInterrupt.
        try:
    ...
    try:
        ...
        logger.exception("Garmin upload failed. No internet?")
...
def coros_upload(player_id, activity):
    try:
        ...
        # `coros_client` is a LOCAL module shipped in the project root (it was
        # added in commit 7c8640b "Add CorosClient"). An ImportError here means
        # either the file is missing on disk or pyinstaller bundled standalone.exe
        # without adding it to `hiddenimports` (see standalone.spec). Print the
        # expected location so the user can fix the install, not just blame
        # "missing pip package".
        expected = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'coros_client.py')
        logger.warning(
            "COROS client module not importable (expected at %s). "
            "Skipping COROS upload: %s" % (expected, repr(exc))
        )
        return
    profile_dir = '%s/%s' % (STORAGE_DIR, player_id)
    ...
    if os.path.exists(creds_file + '.bin'):
        ...
        try:
            ...
            logger.warning("Failed to read %s. Skipping COROS upload attempt: %s" % (creds_file, repr(exc)))
            return
    else:
    ...
    try:
        import hashlib
        # COROS protocol requires raw MD5 of the password (no PBKDF2 etc.);
        # `usedforsecurity=False` opts out of FIPS' MD5 refusal — both digests
        # are wire-format only, not security primitives on our side.
        password_md5 = hashlib.md5(password.encode(), usedforsecurity=False).hexdigest()
        # Don't keep the plaintext password around any longer than needed —
        # it would otherwise be visible in any future exception traceback.
        password = None
        fit_md5 = hashlib.md5(activity.fit, usedforsecurity=False).hexdigest()
        ...
        # Use logger.exception (not logger.warning) so the traceback is
        # preserved — the user needs to be able to tell "no internet" from
        # "OSS sign rotated" from "import API rejected the file".
        logger.exception("COROS upload failed. No internet?")
...
def runalyze_upload(player_id, activity):
    ...
    try:
        r = requests.post("https://runalyze.com/api/v1/activities/uploads",
                          ...
                          timeout=30)
        r.raise_for_status()
        # Truncate the response body in logs — Runalyze sometimes returns
        # multi-KB HTML error pages that just spam zoffline.log.
        logger.info("Runalyze upload response (truncated to 200 chars): %s" % (r.text[:200],))
    except Exception:
        logger.exception("Runalyze upload failed. No internet?")
...
def intervals_upload(player_id, activity):
    ...
    try:
        ...
        # Use the requests params= kwarg rather than string-interpolating
        # activity.name into the URL: a name containing '&' or '?' would
        # otherwise inject extra query params. Also pin to https.
        url = 'https://intervals.icu/api/v1/athlete/%s/activities' % athlete_id
        r = requests.post(
            url,
            files={"file": BytesIO(activity.fit)},
            auth=HTTPBasicAuth('API_KEY', api_key),
            params={'name': activity.name or ''},
            timeout=30,
        )
        r.raise_for_status()
    except Exception:
        logger.exception("Intervals.icu upload failed. No internet?")
...
def zwift_upload(player_id, activity):
    ...
    try:
        # Mount a default 30s timeout on every request the session makes.
        # online_sync.py has no internal timeouts, so without this a single
        # hanging Zwift server would tie up the activity_uploads thread.
        session = _SessionWithTimeout(requests.session(), 30)
        access_token, refresh_token = online_sync.login(session, username, password)
        ...
        logger.exception("Zwift upload failed. No internet?")


class _SessionWithTimeout:
    """Wrap a requests.Session so every request gets a default timeout.

    online_sync.py doesn't set timeouts on its own, so a hung Zwift server
    would tie up the activity_uploads thread forever. Explicit per-call
    timeouts in online_sync would be cleaner, but this wrapper keeps the
    change local and lets existing online_sync code keep working.

    If a caller already passed timeout=... we keep it (don't override).
    """
    def __init__(self, session, default_timeout):
        # Use object.__setattr__ to avoid recursion into our own __getattr__.
...
def api_profiles_activities_id(player_id, activity_id):
    ...
    # daemon=True so a stuck upload (e.g. hung Zwift/Strava server) can't keep
    # the process alive at shutdown — without it, threading.Thread defaults to
    # non-daemon and the main process will block on .join() during interpreter
    # teardown, which is the exact failure mode that masks the real cause.
    upload = threading.Thread(target=activity_uploads, args=(player_id, activity), daemon=True)
    upload.start()
    logout_player(player_id)
    return response, 200
